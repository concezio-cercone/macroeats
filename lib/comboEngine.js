...
function comboExtras(parts) {
  ...
  let sodium = null, fiber = null, sugar = null, addedSugar = null, satFat = null;
...
function buildCombos(restaurants, center, radiusMi, f, opts = {}) {
  ...
  for (const store of restaurants) {
    ...
    for (const m of mainVariants) for (const s of sideVariants) for (const d of dessVariants) {
      ...
      if (f.food && !matchesFood(foodCat + " " + parts.map(p => p.name).join(" ").toLowerCase(), f.food)) continue;

      const modCount = parts.reduce((a, p) => a + p.chosen.length, 0);
      // Prefer combos whose ITEM names actually contain the food term over those
      // that only matched via store category (so "salad" picks the salad, not a
      // chili from a salad-serving chain).
      const foodBonus = (f.food && matchesFood(parts.map(p => p.name).join(" ").toLowerCase(), f.food)) ? 50 : 0;
    ...
    if (best) {
      ...
      out.push({
        ...
        totals: {
          ...
          addedSugar: extras.addedSugar, satFat: extras.satFat,
        },
...
function buildStoreVariants(store, center, f, opts = {}) {
  ...
  for (const mm of mainVariants) for (const s of sideVariants) for (const d of dessVariants) {
    ...
    if (f.carbsMax !== null && sum.carbs > f.carbsMax) continue;
    if (f.wantsCarbs === false && sum.carbs > 30) continue;
    if (f.food && !matchesFood((store.region + " " + (store.cuisines || []).join(" ") + " " + parts.map(p => p.name).join(" ")).toLowerCase(), f.food)) continue;
  ...
  // de-dup identical meals, keep best-scoring, sort, take top `limit`
  const byKey = new Map();
  for (const c of candidates) {
    const ex = byKey.get(c.key);
    if (!ex || c.score < ex.score) byKey.set(c.key, c);
  }
  ...
  return ranked.map(best => ({
    ...
    totals: (() => {
      ...
      return {
        ...
        addedSugar: ex.addedSugar, satFat: ex.satFat,
      };
    })(),
