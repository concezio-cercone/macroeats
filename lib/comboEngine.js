...
function buildPlates(mainVariants, hardHigh) {
  ...
  if (mainVariants.length < 2) return plates;
...
function buildCombos(restaurants, center, radiusMi, f, opts = {}) {
  ...
  for (const store of restaurants) {
    ...
    // Place-specific search: the term matches the restaurant NAME but not its
    // cuisine/items — so "cava"/"chipotle" means that place (show several options
    // from it), whereas "pizza"/"chicken" is a food (one option per store).
    const placeMode = !!f.food
      && matchesFood(store.restaurant.toLowerCase(), f.food)
      && !matchesFood(((store.cuisines || []).join(" ") + " " + store.items.map(i => i.name).join(" ")).toLowerCase(), f.food);
