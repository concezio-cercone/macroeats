...
function buildCombos(restaurants, center, radiusMi, f, opts = {}) {
  ...
  for (const store of restaurants) {
    ...
    // dietary capability: store must support every selected diet tag
    if (opts.diets && opts.diets.length) {
      const sd = (store.diets || []).map(x => x.toLowerCase());
      const ok = opts.diets.every(req => {
    ...
    if (f.food) {
      ...
      if (!hay.includes(f.food)) continue;
    }
    ...
    for (const m of mainVariants) for (const s of sideVariants) for (const d of dessVariants) {
      ...
      const sc = scoreCombo(sum, parts.length, modCount, f, dist, goal);
